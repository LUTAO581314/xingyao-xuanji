...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run BAIRUI",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found BAIRUI session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const executable = process.env.BAIRUI_BIN_PATH ?? process.execPath
          const importResult = yield* Effect.promise(() =>
            Process.text([executable, "import", sessionUrl], { nothrow: true }),
          )
    ...
    UI.println()
    UI.println("Starting BAIRUI...")
    UI.println()

    const bairuiArgs = sessionId ? ["-s", sessionId] : []
    const executable = process.env.BAIRUI_BIN_PATH ?? process.execPath
    const code = yield* Effect.promise(
      () =>
        Process.spawn([executable, ...bairuiArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`BAIRUI exited with code ${code}`))
  }),
})
